...
#import "JNBannerCell.h"

// 总共的item数
#define JN_TOTAL_ITEMS (self.itemCount * 10000)

#define JN_FOOTER_WIDTH 64.0
#define JN_PAGE_CONTROL_HEIGHT 32.0


@interface JNBannerView()<UICollectionViewDataSource, UICollectionViewDelegate>

@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) UICollectionViewFlowLayout *flowLayout;

@property (nonatomic, strong) JNBannerFooter *footer;
...
- (void)updateSubviewsFrame
{
    ...
    self.flowLayout.footerReferenceSize = CGSizeMake(JN_FOOTER_WIDTH, self.frame.size.height);
    ...
    if (CGRectEqualToRect(self.pageControlFrame, CGRectZero)) {
        ...
        CGFloat h = JN_PAGE_CONTROL_HEIGHT;
        CGFloat x = 0;
...
- (void)fixDefaultPosition
{
    ...
    if (self.shouldLoop) {
        // 总item数的中间
        [self.collectionView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:(JN_TOTAL_ITEMS / 2) inSection:0]
...
{
    JNBannerCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:banner_item forIndexPath:indexPath];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)theCollectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)theIndexPath
{
    ...
    if(kind == UICollectionElementKindSectionFooter)
    {
        ...
        self.footer = (JNBannerFooter *)footer;
        
        // 配置footer的提示语
        if ([self.dataSource respondsToSelector:@selector(banner:titleForFooterWithState:)]) {
            self.footer.idleTitle = [self.dataSource banner:self titleForFooterWithState:JNBannerFooterStateNormal];
            self.footer.triggerTitle = [self.dataSource banner:self titleForFooterWithState:JNBannerFooterStateTrigger];
        }
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (footerDisplayOffset > 0) {
        // 开始出现footer
        if (footerDisplayOffset > JN_FOOTER_WIDTH) {
            if (lastOffset > 0) return;
            self.footer.state = JNBannerFooterStateTrigger;
        } else {
            if (lastOffset < 0) return;
            self.footer.state = JNBannerFooterStateNormal;
        }
        lastOffset = footerDisplayOffset - JN_FOOTER_WIDTH;
    }
}
 ...
 *  数据源
 */
- (void)setDataSource:(id<JNBannerViewDataSource>)dataSource {
...
- (void)setCurrentIndex:(NSInteger)currentIndex animated:(BOOL)animated {
    ...
    if (self.shouldLoop) {
        ...
        if(newCurrentIndex >= JN_TOTAL_ITEMS) {
            return;
        }
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        // 注册cell
        [_collectionView registerClass:[JNBannerCell class] forCellWithReuseIdentifier:banner_item];
        
        // 注册 \ 配置footer
        [_collectionView registerClass:[JNBannerFooter class] forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:banner_footer];
        _collectionView.contentInset = UIEdgeInsetsMake(0, 0, 0, -JN_FOOTER_WIDTH);
    }
